...
- retrieve relevant KB sections for query clients or downstream automation
...
The runtime includes a query helper for downstream query clients.
...
- suggest which canonical files the caller should read next
...
```text
Collector -> Sentinel -> Librarian -> canonical KB -> KB runtime -> query clients
```
