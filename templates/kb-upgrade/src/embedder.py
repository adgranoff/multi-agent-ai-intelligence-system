...
from common import append_log, find_api_key, load_json, provider_api_url, save_json, utc_now_iso
...
class Embedder:
    ...
    def _request_embeddings(self, texts: List[str], retry: int = 0) -> Tuple[List[List[float]], int]:
        ...
        try:
            resp = requests.post(provider_api_url("embeddings"), headers=headers, json=payload, timeout=90)
