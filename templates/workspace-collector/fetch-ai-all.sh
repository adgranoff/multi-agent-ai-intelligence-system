...
run_or_placeholder "bash \"$WORKSPACE/fetch-ainews.sh\" > \"$AINEWS_OUT\" 2>/dev/null" "$AINEWS_OUT" "AI News" || true
run_or_placeholder "bash \"$WORKSPACE/fetch-x-digest.sh\" > \"$XDIGEST_OUT\" 2>/dev/null" "$XDIGEST_OUT" "X Digest" || true
run_or_placeholder "bash \"$WORKSPACE/fetch-youtube-digest.sh\" > \"$YOUTUBE_OUT\" 2>/dev/null" "$YOUTUBE_OUT" "YouTube Digest" || true
